...
function recordToJob(r: Record<string, unknown>): Job {
  ...
  return {
    ...
    visaSponsorshipNote: f.VisaSponsorshipNote as string | undefined,
    contractType: f.ContractType as string | undefined,
    workingPattern: f.WorkingPattern as string | undefined,
    keyStages: f.KeyStages as string | undefined,
...
export async function createJob(job: Omit<Job, "id">): Promise<Job> {
  const data = await airtableFetch(JOBS_TABLE, "POST", {
    ...
    fields: {
      ...
      VisaSponsorshipNote: job.visaSponsorshipNote,
      ContractType: job.contractType,
      WorkingPattern: job.workingPattern,
      KeyStages: job.keyStages,
      Status: job.status,
      Notes: job.notes,
